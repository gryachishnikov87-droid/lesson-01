...
// 2. Инициализируем переменную для хранения индекса текущего изображения
let currentIndex = 0;

// 3. Функция для обновления атрибута src у изображения
function updateImage() {
  imageElement.src = web_tech_images[currentIndex];
}

// 4. Обработчик для кнопки "next" (вперед)
nextButton.addEventListener('click', () => {
  currentIndex++;
  // Если вышли за пределы массива, переходим к первому элементу (индекс 0)
...
// 5. Обработчик для кнопки "prev" (назад)
